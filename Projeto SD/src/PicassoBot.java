...
public class PicassoBot {
	...
	private void printTiles() {
		try {
			...
			URL url = new URL("http://ucx.dei.uc.pt/game/board");
			...
			for (int i=0;i<nodes.getLength();i++) {
				...
				// Fetching the atributes of the item element
	...
	private boolean paintDot(int x, int y, int r, int g, int b) {
		try {
			URL url = new URL("http://ucx.dei.uc.pt/game/board/" + x + "/" + y);
	        HttpURLConnection connection = (HttpURLConnection) url.openConnection();
	        ...
	        // We can use getOutputStream() for passing the values for r, g and b in the request's body
